...
public class LootDropper : MonoBehaviour
{
    ...
    public static int totalLootCount = 1; // Static counter for total loot items
        ...
        foreach (var loot in lootTable)
        {
            if (Random.value <= loot.dropChance)
            {
                ...
                if (lootScript != null)
                {
                    ...
                    totalLootCount++; // Increment the total loot count
                    Debug.Log($"Total Loot Count: {totalLootCount}"); // Debug log for loot count
                }
                else
                {
